...
const tokensUtiles = (nombre) => norm(nombre.replace(/\([^)]*\)/g, ' ')).split(' ').filter((w) => w && !JUNK.has(w) && !/^\d+([.,]\d+)?$/.test(w));
const intentar = (txt) => {
  const nk = norm(txt);
  if (!nk) return null;
  ...
  if (byKey.has(key(nk))) return { r: byKey.get(key(nk)), m: 'tokens' };
...
const resolver = (nombre) => {
  const toks = tokensUtiles(nombre);
  for (let n = Math.min(4, toks.length); n >= 1; n--) {
    for (let i = 0; i + n <= toks.length; i++) {
      const cand = toks.slice(i, i + n).join(' ');
      const r = intentar(cand);
      if (r) return { ...r, txt: cand, ventana: n + ' tok desde ' + i };
    }
  }
  return null;
};
...
for (const p of pend) {
  let hit = null;
  if (p.molecula) hit = resolver(p.molecula) || null;
...
fs.writeFileSync(path.join(DATA, '2026-09-25_auto_resueltos_ronda3.csv'),
  ['producto_id,nombre,molecula_usada,ventana,ref_id,ref_nombre,atc_id,metodo,resultado'].join('\n') + '\n' +
  log.map((r) => [r.id, r.nombre_comercial, r.hit.txt, r.hit.ventana || '', r.hit.r.id, r.hit.r.nombre, r.hit.r.atc_id || '', r.hit.m, r.res].map(q).join(',')).join('\n') + '\n', 'utf-8');
...
console.log('aplicados:', ok, '| propuestos(tipo typo):', propuestos.length, '| para ti:', quedan.length);
for (const r of log.filter((x) => x.res === 'INSERTADO')) console.log('  + ' + String(r.id).padStart(6) + '  ' + r.nombre_comercial.substring(0, 48).padEnd(48) + ' "' + r.hit.txt + '" -> ' + r.hit.r.nombre + ' [' + (r.hit.r.atc_id || 'sinATC') + ']');
