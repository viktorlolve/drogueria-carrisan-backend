...
await c.connect();
// 1) revertir el complejo vitaminico (candidata nueva, no un solo componente)
await c.query("DELETE FROM producto_moleculas WHERE producto_id=39869");
console.log('39869 revertido (complejo B1+B2+B6+B12 -> candidata nueva)');
...
const quedan = [], props = [];
for (const p of pend) { const h = resolver(p.molecula || '') || resolver(p.nombre_comercial); if (h && h.prop) props.push({ ...p, h }); else if (!h) quedan.push(p); }
// 2) el dueno ya lo respondio -> solo confirmar typos
...
// marcar en la auditoria de ronda4 el revertido
const a4 = path.join(DATA, '2026-09-25_auto_resueltos_ronda4.csv');
fs.writeFileSync(a4, fs.readFileSync(a4, 'utf8').replace(/^39869,.*INSERTADO$/m, (m) => m.replace(/INSERTADO$/, 'REVERTIDO_COMPLEJO')), 'utf-8');
console.log('propuestas typo:', props.map((p) => p.id + ' ' + p.nombre_comercial.substring(0,34)).join(' | '));
console.log('PARA TI:', quedan.length);
